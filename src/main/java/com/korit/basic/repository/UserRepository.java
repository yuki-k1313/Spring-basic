...
public interface UserRepository extends JpaRepository<UserEntity, String> {
    ...
    // - findBy : [SELECT * WHERE] : 필드명을 기준으로 모든 컬럼을 조회할 때 사용함, 
    //            findBy 바로 뒤에 필드명을 붙여서 작성, 필드명은 반드시 첫 글자가 대문자로 작성되어야 함
    
    // SELECT * FROM user WHERE user_name = ?;
    // List 타입으로 반환 받는 경우 결과가 없으면 길이가 0인 리스트를 반환
    ...
    // Like, NotLike, Containing, StratingWith, EndingWith :
    ...
    @Query(value="SELECT u FROM user u WHERE u.user_name = ?1 AND u.usertelNumber = ?2")
    List<UserEntity> getUserList(String userName, String userTelNumber);

    @Query(value="SELECT u FROM user u WHERE u.user_name = :name AND u.usertelNumber = :telNumber")
